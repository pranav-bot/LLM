...
def prepreocess(df, eda_results):
    ...
    for column in datetime_columns:
        try:
            if df[column].dt.year.notna().any():
                df[column+' Year'] = df[column].dt.year
            if df[column].dt.month.notna().any():
                df[column+' Month'] = df[column].dt.month
            if df[column].dt.day.notna().any():
                df[column+' Day'] = df[column].dt.day
            if df[column].dt.dayofweek.notna().any():
